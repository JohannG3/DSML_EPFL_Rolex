...
def main():
    ...
    sentence = st.text_input("Enter a sentence in French", key=f"sentence_{cycle_key}")
    if st.button('Analyze the sentence', key=f"predict_{cycle_key}"):
        ...
        if f"current_prediction_{cycle_key}" in st.session_state:
            new_sentence = st.text_input("Enter a new sentence to try to improve you french level and test your learning!", key=f"improved_{cycle_key}")
            if st.button('Submit the improved sentence', key=f"submit_improved_{cycle_key}"):
                ...
                if new_difficulty > st.session_state[f"current_prediction_{cycle_key}"]:
                    st.success("Congratulations ! The difficulty level of your sentence has increased.")
                    if st.button('Start again with a new sentence', key=f"new_sentence_{cycle_key}"):
